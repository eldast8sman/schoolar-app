...
use App\Http\Resources\SchoolSessionResource;
...
class SessionController extends Controller
{
    ...
    public function __construct(SessionRepositoryInterface $interface)
    {
    ...
    public function index(){
        ...
        $sessions = $this->interface->index($search, $limit, $filter);

        return $this->success_response("Sessions fetched successfully", SchoolSessionResource::collection($sessions)->response()->getData(true));
    }
    ...
    public function store(StoreSesssionRequest $request){
        if(!$session = $this->interface->store($request)){
            return $this->failed_response($this->interface->errors, 400);
        }

        return $this->success_response("Session successfully added", new SchoolSessionResource($session));
    }
    ...
    public function update(UpdateSessionRequest $request, SchoolSession $session){
        if(!$updated = $this->interface->update_session($request, $session)){
            return $this->failed_response($this->interface->errors, 409);
        }
        return $this->success_response("Session Updated successfully", new SchoolSessionResource($updated));
    }
    ...
    public function destroy(SchoolSession $session){
        if(!$this->interface->destroy($session)){
            return $this->failed_response($this->interface->errors, 409);
        }
        return $this->success_response("Session successfully deleted");
    }
